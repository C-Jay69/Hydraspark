...
export const swipe = api<SwipeRequest, SwipeResponse>(
  { expose: true, method: "POST", path: "/matching/swipe" },
  async ({ userId, targetUserId, direction }) => {
    ...
    const vibeScore = calculateVibeScore(
      currentUser.vibe_answers || {},
      targetUser.vibe_answers || {}
    );
    ...
    if (direction === "right" || direction === "super") {
      const mutualSwipe = await matchingDB.queryRow<{ direction: string }>`
        SELECT direction
        FROM swipes
      ...
      if (mutualSwipe && (mutualSwipe.direction === "right" || mutualSwipe.direction === "super")) {
        ...
        const match = await matchingDB.queryRow<{ id: number }>`
...
// Gets user's matches.
export const getMatches = api<{ userId: string }, { matches: Array<{
  id: number;
  user: {
    id: string;
    firstName: string;
  ...
  { expose: true, method: "GET", path: "/matching/matches/:userId" },
  async ({ userId }) => {
