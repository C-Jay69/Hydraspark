...
export const addTrustedContact = api({ expose: true, method: "POST", path: "/safety/trusted-contacts" }, async (req) => {
    // Limit to 3 trusted contacts
    const existingCount = await safetyDB.queryRow `
...
export const getTrustedContacts = api({ expose: true, method: "GET", path: "/safety/trusted-contacts/:userId" }, async ({ userId }) => {
...
export const removeTrustedContact = api({ expose: true, method: "DELETE", path: "/safety/trusted-contacts/:userId/:contactId" }, async ({ userId, contactId }) => {
...
async function recordSafetyAction(userId, actionType, points) {
    await safetyDB.exec `
...
//# sourceMappingURL=trusted_contacts.js.map