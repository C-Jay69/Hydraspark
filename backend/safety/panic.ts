...
export const triggerPanic = api<TriggerPanicRequest, PanicAlert>(
  { expose: true, method: "POST", path: "/safety/panic" },
  async (req) => {
...
export const resolvePanic = api<{ userId: string; alertId: number; status: "resolved" | "false_alarm" }, void>(
  { expose: true, method: "PUT", path: "/safety/panic/:userId/:alertId/resolve" },
  async ({ userId, alertId, status }) => {
    const result = await safetyDB.queryRow<{ id: number }>`
      UPDATE panic_alerts 
...
export const getPanicAlerts = api<{ userId: string }, { alerts: PanicAlert[] }>(
  { expose: true, method: "GET", path: "/safety/panic-alerts/:userId" },
  async ({ userId }) => {
