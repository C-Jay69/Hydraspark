...
import { auth, AuthData } from "encore.dev/auth";
...
export const sendMessage = api.POST("/chat/messages", async ({ recipientId, text }: SendMessageParams): Promise<void> => {
...
export const getMessages = api.GET("/chat/messages/:recipientId", async ({ recipientId }: GetMessagesParams): Promise<GetMessagesResponse> => {
    const { id } = await auth.data();
...
export const messageStream = api.GET("/chat/messages/:recipientId/stream", async function* ({ recipientId }: MessageStreamParams): AsyncGenerator<stream<Message>> {
    const { id } = await auth.data();
    ...
    `) {
        yield stream.data(message as Message);
    }
});
