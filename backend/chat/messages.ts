...
export const getMessages = api<{ matchId: number; userId: string; limit?: number; offset?: number }, { messages: ChatMessage[] }>(
  { expose: true, method: "GET", path: "/chat/messages/:matchId/:userId" },
  async ({ matchId, userId, limit = 50, offset = 0 }) => {
...
export const getIcebreakers = api<void, { icebreakers: Array<{
  id: number;
  title: string;
  description: string;
  category: string;
  ...
  { expose: true, method: "GET", path: "/chat/icebreakers" },
  async () => {
...
export const sendIcebreaker = api<{
  matchId: number;
  senderId: string;
  icebreakerId: number;
  selectedPrompt: string;
}, ChatMessage>(
  { expose: true, method: "POST", path: "/chat/icebreaker" },
  async ({ matchId, senderId, icebreakerId, selectedPrompt }) => {
