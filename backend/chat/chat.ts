...
interface Message {
  ...
  sender: string;
  timestamp: string;
  conversationID: string; // Add conversationID to the message
}
...
export async function GetMessages(params: GetMessagesParams): Promise<{ messages: Message[] }> {
...
export const NewMessages = topic<Message>('new-messages', {
